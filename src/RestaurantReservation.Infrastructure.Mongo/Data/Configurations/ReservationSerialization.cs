    ...
    public static void Register()
    {
        BsonClassMap.RegisterClassMap<Reservation>(
            map =>
            {
                ...
                map.MapProperty(x => x.TableId)
                    .SetSerializer(new IdSerializationProvider<TableId>(new GuidSerializer(BsonType.String)));
                map.MapProperty(x => x.ReviewId)
                    .SetSerializer(new IdSerializationProvider<ReviewId>(new GuidSerializer(BsonType.String)))
                    .SetDefaultValue("");
            });
    }
}
